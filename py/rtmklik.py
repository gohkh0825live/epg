...
def get_formatted_channel_id(channel_name):
    ...
    return f"{clean_id or 'CHANNEL'}.rtm"


def parse_time(dt_str):
    """将时间转换为标准 XMLTV 格式 (YYYYMMDDHHMMSS +0800)"""
    if not dt_str:
        return ""
    try:
...
def json_to_xmltv(combined_channels_data):
    ...
    for name, programmes in combined_channels_data.items():
        ...
        for prog in programmes:
            ...
            title_text = (
                prog_info.get("title") or prog.get("title") or "Untitled"
            )
            desc_text = (
                prog_info.get("description") or prog.get("description") or ""
            )
...
if __name__ == "__main__":
    ...
    # 用于存放合并后的频道节目数据 { channel_name: [programme_list] }
    combined_channels = {name: [] for name in CHANNEL_IDS.keys()}

    for date_str in dates_to_fetch:
        print(f"正在抓取 [{date_str}] 的 RTM EPG 数据...")
        ...
        for name, cid in CHANNEL_IDS.items():
            ...
            try:
                ...
                progs = (
                    raw.get("data", raw) if isinstance(raw, dict) else raw
                )
    ...
    if any(combined_channels.values()):
        try:
            ...
            with open("rtm_epg.xml", "w", encoding="utf-8") as f:
                f.write(xml_content)
            print(f"\n✅ 成功生成多日合并的 XMLTV 文件: rtmklik.xml")
        except Exception as e:
            print(f"❌ 生成 XML 失败: {e}")
            exit(1)
    else:
